...
import { InternalDocument, InternalDocumentFilter } from "@/types/InternalDocument";
...
export default function DocumentInternalOutgoingPage() {
    const { hasInternalDocumentRight } = useAuthContext();
    const [data, setData] = useState<InternalDocument[]>([]);
    const [filter, setFilter] = useState<InternalDocumentFilter>({ documentType: DocType.InternalOutgoing, pageNumber: 1, pageSize: 10 });
        ...
        ...ColumnsData,
        {
            ...
            cell: ({ row }) => (
                <div className="flex space-x-2 justify-center">
                    <Link href={`${PATH.DocumentInternalOutgoing}/${row.original.id}`} target="_blank">
                        <Button variant="outline" size="sm" className="h-8 px-2 py-0">
                            <Eye size={14} />
                        </Button>
                    </Link>
                    {hasInternalDocumentRight&&<Button onClick={() => setOpenDeleteDialog({ open: true, id: row.original.id, name: row.original.name })} variant="outline" size="sm" className="h-8 px-2 py-0">
                        <Trash size={14} />
                    </Button>}
                </div>
            ),
    ...
    return (
        <div>
            <PageHeader title="Văn bản nội bộ đi" subtitle="Danh sách văn bản nội bộ đi trong công ty">
                {hasInternalDocumentRight&&<Link href={`${PATH.DocumentInternalOutgoing}/create`} target="_blank">
