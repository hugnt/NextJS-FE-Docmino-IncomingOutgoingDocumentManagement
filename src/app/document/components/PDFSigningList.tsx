...
import { ImageSignature } from '@/types/ConfirmProcess';
import { DocumentFile } from '@/types/DocumentFile';
import { Signature } from 'lucide-react';

interface PDFSigningListProps {
    pdfFiles: DocumentFile[];
    handlePDFSigningViewer: (file: DocumentFile) => void;
    imageSignatures?: ImageSignature[];
    loading?: boolean;
}

export default function PDFSigningList(props: PDFSigningListProps) {
    const { pdfFiles, handlePDFSigningViewer, imageSignatures, loading = false } = props;
    return (
            ...
            <Table>
                <TableHeader>
                    <TableRow className="bg-muted/30 hover:bg-muted/30 border-t">
                        ...
                        <TableHead className="w-[100px] font-medium">Trạng thái</TableHead>
                ...
                <TableBody className='border-b'>
                    {pdfFiles!.map((file, i) => (
                        <TableRow key={file.id} className="hover:bg-muted/10">
                            ...
                            <TableCell>
                                <Badge className={cn("text-xs", imageSignatures?.find(x=>x.fileId == file.id)?.isVerify ? "bg-green-500" : "bg-yellow-500")}>
                                    {imageSignatures?.find(x=>x.fileId == file.id)?.isVerify ? "Đã xác nhận" : "Chưa xác nhận"}
                                </Badge>
                            </TableCell>
                            <TableCell>
                                <div className="flex justify-center gap-1">
                                    <Button
                                        onClick={() => handlePDFSigningViewer(file)}
                                        type="button"
                                        variant="ghost"
                                        size="icon"
