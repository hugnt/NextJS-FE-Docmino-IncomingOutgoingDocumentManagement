...
export default function StoragePeriodList() {
    const [data, setData] = useState<StoragePeriod[]>([]);
    ...
    const handleGetList = () => {
        ...
        storagePeriodRequest.getByFilter(filter).then(res => {
            setData(res.data || []);
    ...
    //FORM HANDLER
    const handleFormAction = (mode: FormMode, row?: Row<StoragePeriod>) => {
        if (mode == FormMode.ADD) {
            setDetail(defaultStoragePeriod);
        }
        else if (row && mode == FormMode.EDIT) {
            const id = row.original.id;
            storagePeriodRequest.getById(id).then(res => {
                setDetail(res.data);
            });
    ...
    const handleFormSubmit = (data: StoragePeriod) => {
        if (formSetting.mode == FormMode.ADD) {
            storagePeriodRequest.create(data).then(res => {
                handleSuccessApi({ title: "Thêm mới thành công!", message: res.message })
                handleGetList();
            });
        }
        else if (formSetting.mode == FormMode.EDIT) {
            storagePeriodRequest.update(detail!.id, data).then(res => {
    ...
    const handleConfirmDelete = () => {
        storagePeriodRequest.delete(openDeleteDialog.id).then(res => {
