...
export default function OrganizationList() {
    ...
    const [detail, setDetail] = useState<Organization>();
    ...
    const handleGetList = () => {
        ...
        organizationRequest.getByFilter(filter).then(res => {
            setData(res.data || []);
    ...
    //FORM HANDLER
    const handleFormAction = (mode: FormMode, row?: Row<Organization>) => {
        if (mode == FormMode.ADD) {
            setDetail(defaultOrganization);
        }
        else if (row && mode == FormMode.EDIT) {
            const id = row.original.id;
            organizationRequest.getById(id).then(res => {
                setDetail(res.data);
            });
    ...
    const handleConfirmDelete = () => {
        organizationRequest.delete(openDeleteDialog.id).then(res => {
    ...
    return (
        <div>
            <PageHeader title="Tổ chức" subtitle="Quản lý tổ chức">
            ...
            <ConfirmDialog
                ...
                className='max-w-md'
                title={`Delete this organization: ${openDeleteDialog.name} ?`}
                desc={
                    <>
                        Bạn sắp xóa tổ chức với tên{' '}
