...
import documentRegisterRequest from "@/api/documentRegisterRequest";
...
export default function DocumentRegisterList() {
    const [data, setData] = useState<DocumentRegister[]>([]);
    ...
    const handleGetList = () => {
        ...
        documentRegisterRequest.getByFilter(filter).then(res => {
            setData(res.data || []);
    ...
    //FORM HANDLER
    const handleFormAction = (mode: FormMode, row?: Row<DocumentRegister>) => {
        if (mode == FormMode.ADD) {
            setDetail(defaultDocumentRegister);
        }
        else if (row && mode == FormMode.EDIT) {
            const id = row.original.id;
            documentRegisterRequest.getById(id).then(res => {
                setDetail(res.data);
            });
    ...
    const handleFormSubmit = (data: DocumentRegister) => {
        if (formSetting.mode == FormMode.ADD) {
            documentRegisterRequest.create(data).then(res => {
                handleSuccessApi({ title: "Thêm mới thành công!", message: res.message })
                handleGetList();
            });
        }
        else if (formSetting.mode == FormMode.EDIT) {
            documentRegisterRequest.update(detail!.id, data).then(res => {
    ...
    const handleConfirmDelete = () => {
        documentRegisterRequest.delete(openDeleteDialog.id).then(res => {
    ...
    return (
        <div>
            <PageHeader title="Sổ đăng ký văn bản" subtitle="Quản lý sổ đăng ký văn bản với các loại văn bản đi/đến/nội bộ">
            ...
            <ConfirmDialog
                ...
                className='max-w-md'
                title={`Delete this register: ${openDeleteDialog.name} ?`}
                desc={
                    <>
                        Bạn sắp xóa sổ đăng ký với tên{' '}
