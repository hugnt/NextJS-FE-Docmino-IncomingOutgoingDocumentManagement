...
export default function BoxPage() {
  ...
  const sheftId = searchParams.get(PARAM.SHEFT_ID);

  const { refeshTree, tree, getShelf } = useDirectoryContext();
  const {hasStoreDocumentRight} = useAuthContext();
  const [data, setData] = useState<DocumentDirectory[]>([]);
  const [filter, setFilter] = useState<DocumentDirectoryFilter>({ parentDirectoryId: sheftId, type: DirectoryType.Box, pageNumber: 1, pageSize: 5 });
  const [totalRecords, setTotalRecords] = useState<number>(0);
  const [formSetting, setFormSetting] = useState<FormSetting>(formSettingDefault);
  const [detail, setDetail] = useState<DocumentDirectory>({ ...defaultDocumentDirectory, parentDirectoryId: sheftId, type: DirectoryType.Box });
  ...
  useEffect(() => {
    if (sheftId) {
      setFilter({ ...filter, parentDirectoryId: sheftId });
      setDetail({ ...defaultDocumentDirectory, parentDirectoryId: sheftId, type: DirectoryType.Box });
    }
    else {
      setFilter({ ...filter, parentDirectoryId: null });
      setDetail({ ...defaultDocumentDirectory, parentDirectoryId: null, type: DirectoryType.Box });
    }
  ...
  const handleFormAction = (mode: FormMode, selectedItem?: DocumentDirectory) => {
    if (mode == FormMode.ADD) setDetail({ ...defaultDocumentDirectory, parentDirectoryId: sheftId, type: DirectoryType.Box });
    else if (selectedItem && mode == FormMode.EDIT) setDetail(selectedItem);
    else if (selectedItem && mode == FormMode.REDIRECT) {
      router.push(`${PATH.SystemCategoryStorageDossier}?${PARAM.BOX_ID}=${selectedItem.id}`);
      return;
    }
    setFormSetting({
      mode: mode,
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-gray-50">
      <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b bg-white px-4 md:px-6">
        {sheftId && <Link href={`${PATH.SystemCategoryStorageSheft}?${PARAM.INVENROTY_ID}=${getShelf(sheftId)?.parentDirectoryId}`} className="flex items-center">
          <Button variant="outline" size="sm">
            <ChevronLeft className="mr-2 h-4 w-4" />
            Quay lại danh sách kệ
          </Button>
        </Link>}
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">
            Hộp Lưu trữ
            {sheftId && <Badge className="ml-2 bg-blue-500">
              {getShelf(sheftId)?.name}
            </Badge>}
          </h1>
        </div>
        <div className="ml-auto flex items-center gap-4">
          {hasStoreDocumentRight&&<Button onClick={() => handleFormAction(FormMode.ADD)} className='space-x-1'>
            <span>Thêm hộp lưu trữ</span><Plus size={18} />
          </Button>}
        </div>
      </header>
