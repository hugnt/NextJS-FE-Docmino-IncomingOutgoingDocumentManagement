...
import { getDossierStatusBadge } from "@/lib/directoryHelper"
import { formatDate } from "@/lib/utils"
import { type Dossier } from "@/types/Dossier"
import { Calendar, CalendarCheck, Clock, FileText, Folder, MoreVertical, Package, PackageSearch, Pen, Trash } from "lucide-react"

interface DossierGridProps {
    data: Dossier[]
    handleDetailClick: (detail: Dossier) => void
    handleEditClick: (detail: Dossier) => void
    handleDeleteClick: (detail: Dossier) => void
    loading?: boolean
}

export default function DossierGrid(props: DossierGridProps) {
                ...
                <Card
                    ...
                    <CardContent onClick={() => handleDetailClick(item)}
                        ...
                        <div className="grid grid-rows-5 mb-2">
                            ...
                            {item.boxId && <div className="flex items-center gap-2 truncate">
                                <Package className="h-4 w-4 text-muted-foreground" />
                                <span className="text-xs text-muted-foreground">{item.boxName}</span>
                            </div>}
                            ...
                            <div className="flex items-center gap-2 text-xs text-muted-foreground truncate">
                                ...
                                <span>{item.storagePeriodName}</span>
                            </div>
                        </div>
