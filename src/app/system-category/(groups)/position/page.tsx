...
export default function PositionList() {
    ...
    const handleGetList = () => {
        ...
        positionRequest.getByFilter(filter).then(res => {
            setData(res.data || []);
    ...
    //FORM HANDLER
    const handleFormAction = (mode: FormMode, row?: Row<Position>) => {
        if (mode == FormMode.ADD) {
            setDetail(defaultPosition);
        }
        else if (row && mode == FormMode.EDIT) {
            const id = row.original.id;
            positionRequest.getById(id).then(res => {
                setDetail(res.data);
            });
    ...
    const handleConfirmDelete = () => {
        positionRequest.delete(openDeleteDialog.id).then(res => {
    ...
    return (
        <div>
            <PageHeader title="Chức vụ" subtitle="Quản lý chức vụ">
            ...
            <ConfirmDialog
                ...
                className='max-w-md'
                title={`Delete this position: ${openDeleteDialog.name} ?`}
                desc={
                    <>
                        Bạn sắp xóa chức vụ với tên{' '}
