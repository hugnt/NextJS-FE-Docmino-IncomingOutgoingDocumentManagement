...
export default function DepartmentList() {
    ...
    const handleGetList = () => {
        ...
        departmentRequest.getByFilter(filter).then(res => {
            setData(res.data || []);
    ...
    //FORM HANDLER
    const handleFormAction = (mode: FormMode, row?: Row<Department>) => {
        if (mode == FormMode.ADD) {
            setDetail(defaultDepartment);
        }
        else if (row && mode == FormMode.EDIT) {
            const id = row.original.id;
            departmentRequest.getById(id).then(res => {
                setDetail(res.data);
            });
    ...
    const handleConfirmDelete = () => {
        departmentRequest.delete(openDeleteDialog.id).then(res => {
    ...
    return (
        <div>
            <PageHeader title="Phòng ban" subtitle="Quản lý phòng ban">
            ...
            <ConfirmDialog
                ...
                className='max-w-md'
                title={`Delete this department: ${openDeleteDialog.name} ?`}
                desc={
                    <>
                        Bạn sắp xóa phòng ban với tên{' '}
